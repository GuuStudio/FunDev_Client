...
import PaginationProfile from "~/services/Paging/PaginationProfile";
...
function ProfileProducts() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      await api.get('/api/Products').then((response) => response.data.filter(item => item.userId === localStorage.getItem("userId")))
